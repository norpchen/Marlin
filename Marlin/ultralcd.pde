...
#define OLED_DC 9 //MISO_PIN // 11
#define OLED_CS SPI_DISPLAY_CS //12
//#define OLED_CLK SCK_PIN //10
//#define OLED_MOSI MOSI_PIN // 9
#define OLED_RESET 40 // 13
...
#define I2C_ADDR    0x27 // 27  // Define I2C Address where the PCF8574A is
...
// LiquidCrystal_I2C lcd(0x27, 2, 1, 0, 4, 5, 6, 7, 3, );  // Set the LCD I2C address
...
void lcd_init()
{
...
#else
    lcd.begin(SSD1306_SWITCHCAPVCC);  // initialize with the I2C addr 0x3D (for the 128x64)
	lcd.display();
    ...
    lcd.setCursor(0,0);
    //lcd.println ("MOFONGO "__TIMESTAMP__);
    //lcd.println ("MEM " +String (freeMemory()));
...
#define LED_C 0x01
    ...
    if (last_message_priority>0)
        ...
        if (last_message_priority<0) last_message_priority = 1;
...
MainMenu::MainMenu()
{
    last_drawn_progress_string[0]="";
    last_drawn_progress_string[1]="";
    last_drawn_progress_string[2]="";
...
if (reentracy_blocker) return;
reentracy_blocker = true;
...
//	return;
    ...
    if (state!=NULL)
        {
            ...
            if (state.Controller()==SDCARD) lcd.print ("\006");
                ...
                if (state.Controller()==SERIAL_HOST) lcd.print ("\007");
                else lcd.print ("\008");		// panel
#endif
            int s = (int) ((STATES) state);
            if (s >= (int) IDLE && s < (int) MAX_STATES && force_lcd_update)
                // lcd.print (String("STATE: ") + String (s) );
                lcd.print (STATE_STRINGS[s]);
        }
    ...
    if (progress_string[0]!=last_drawn_progress_string[0] || force_lcd_update)
        {
            //	lcd.setCursor(0,2*CHAR_HEIGHT);
            //	lcd.print ("         ");
            lcd.setCursor(0,2*CHAR_HEIGHT);

            lcd.print(progress_string[0]);//+String(" ")  +progress_string[2]);
    ...
    if (progress_string[1]!=last_drawn_progress_string[1]|| force_lcd_update)
        {
            //	lcd.setCursor(CHAR_WIDTH*10,1*CHAR_HEIGHT);
            //	lcd.print ("         ");
            lcd.setCursor(CHAR_WIDTH*10,1*CHAR_HEIGHT);
    ...
    if (progress_string[2]!=last_drawn_progress_string[2]|| force_lcd_update)
        {
            //	lcd.setCursor(11,2);
            //	lcd.print ("         ");
            lcd.setCursor(CHAR_WIDTH*11,2*CHAR_HEIGHT);
    ...
    if(messagetext[0]!='\0')
        {
            lcd.setCursor(0,CHAR_HEIGHT*(LCD_HEIGHT-1));
            //	lcd.print ("                      ");
            //		lcd.setCursor(0,CHAR_HEIGHT*(LCD_HEIGHT-1));
