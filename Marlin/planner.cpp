...
// TODO:  This looks like a good place to hook in the motion LEDs
...
void plan_buffer_line(const float &x, const float &y, const float &z, const float &e, float feed_rate, const uint8_t &extruder, float q)
{
    ...
    float speed_factor = q;//1.0; //factor <=1 do decrease speed
    for(int i=0; i < 4; i++)
        {
...
#define MAX_FREQ_TIME (1000000.0/XY_FREQUENCY_LIMIT)
    ...
    if(block->steps_x == 0 && block->steps_y == 0 && block->steps_z == 0)
        ...
        {
            block->acceleration_st = ceil(q * acceleration * steps_per_mm); // convert to: acceleration steps/sec^2
    ...
    float vmax_junction = q*max_xy_jerk/2;
    ...
    if ((moves_queued > 1) && (previous_nominal_speed > 0.0001))
        {
            ...
            //    }
            if (jerk > q*max_xy_jerk)
                {
                    vmax_junction_factor = (q*max_xy_jerk/jerk);
                }
