...
// the first four values were taken from a known table.  Everything at 260 or below was measured.
// on a Printrbot Jr. which is a 100K Enco -- every resistor will be slightly different, so it's best
// to calibrate based your exact hardware: 
// Hook up a trusted temperature sensor to the hot end and record the output
// of the raw resistance, then create a table from it by taking 4*(4096-resistance)
// and the temperature in 'C
// Make sure to let the PID and external thermometer stabilize for a while for each reading.
...
const short temptable_99[][2] PROGMEM ={
 ...
 #endif
  // How often to log the raw resistance values for temperature calibration
 // Lower numbers is more often, but below 30 seems to cause lockups (thread lock?) 
 // set to zero to disable
 // 
 // TODO: add support for this
  #define RAW_RESISTANCE_OUTPUT_PERIOD 10000
