...
float BBLookAheadHot = 4;
float BBLookAheadCool = 8;
...
void PID_autotune(float temp, int extruder, int ncycles)
	...
	for(;;)
		...
		if(cycles > ncycles)
			{
			SERIAL_PROTOCOLLNPGM("PID Autotune finished ! Place these final Kp, Ki and Kd constants in the configuration.h");
			SERIAL_PROTOCOLPGM(" Kp: ");
			SERIAL_PROTOCOLLN(Kp);
			SERIAL_PROTOCOLPGM(" Ki: ");
			SERIAL_PROTOCOLLN(Ki);
			SERIAL_PROTOCOLPGM(" Kd: ");
			SERIAL_PROTOCOLLN(Kd);
			//gcode_execute_PString ("M206 T3 X",Kp," P218\nM206 T3 X",Ki," P222\nM206 T3 X",Kd," P226");
...
#define K2 (1.0-K1)
	...
	// track which way the temperature is moving, and predict the next update
	static int last_bed_temp = -1;
	if (last_bed_temp < bed_minttemp) last_bed_temp = current_raw_bed;
	int delta_bed_temp = current_raw_bed - last_bed_temp;
	if (delta_bed_temp > 0 )
		delta_bed_temp *= BBLookAheadHot;
	else
		delta_bed_temp *= BBLookAheadCool;
	last_bed_temp = current_raw_bed;

	// Check if temperature is within the correct range
	if((current_raw_bed > bed_minttemp) && (current_raw_bed < bed_maxttemp))
		{
		// if the heater is on, check if the temp will be over the target after X cycles based on the current rate of change,
		if (soft_pwm_bed!=0)
			{
			if(current_raw_bed+delta_bed_temp >= target_raw_bed)		// we're going to overshoot, turn off heater.
				{
				soft_pwm_bed = 0;
				}
			else
				{
				soft_pwm_bed = MAX_BED_POWER>>1;				// no overshoot, turn on / leave on the heater
				}
			}
		else
			{
			// if heater is off, check if the rate of cooling will put us below the target temperature after Y cycles, based on the current rate of change.
			if(current_raw_bed+delta_bed_temp <= target_raw_bed)
				{
				soft_pwm_bed = MAX_BED_POWER>>1;				// yes, we're going to dip below, fire up the heaters
				}
			else
				{
				soft_pwm_bed = 0;					// no, leave it off
				}
			}
		}
	else
