...
int frequently_updated_offset =256;
...
void EEPROM_StoreSettings()
{
    ...
    EEPROM_writeAnything(i,BBLookAheadHot);
    EEPROM_writeAnything(i,BBLookAheadCool);


    while (frequently_updated_offset < i) frequently_updated_offset+=256;
...
#endif
    SERIAL_ECHO_START;
    SERIAL_ECHOLNPGM("Bed Bang Bang Look Ahead Settings:");
    SERIAL_ECHO_START;
    SERIAL_ECHOPAIR(" M310 H" ,BBLookAheadHot);
    SERIAL_ECHOPAIR(" C" ,BBLookAheadCool);
    SERIAL_ECHOLN("");
...
void EEPROM_RetrieveSettings(bool def)
{
    ...
    if ((!def)&&(strncmp(ver,stored_ver,3)==0)||(strncmp("V08",stored_ver,3)==0))
        {
            ...
            if (strncmp(ver,stored_ver,3)==0)
                {
                    EEPROM_readAnything(i,BBLookAheadHot);
                    EEPROM_readAnything(i,BBLookAheadCool);


                    while (frequently_updated_offset < i) frequently_updated_offset+=256;
                    i = frequently_updated_offset;
                }
